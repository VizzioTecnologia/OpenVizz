...
class Unid_api_common_model extends CI_Model{
	...
	public function api( $config, $internal_mode = NULL, $_skip_access_check = NULL ) {
		...
		if ( ! isset( $act ) ) {
			...
			// URL / POST
				...
				$pp =										isset( $post[ 'pp' ] ) ?  urldecode( $post[ 'pp' ] ) : ( isset( $config[ 'pp' ] ) ? urldecode( $config[ 'pp' ] ) : FALSE ); // Parse UniD Data Property
											...
											if ( isset( $api_access[ 'errors' ] ) ) {
												...
												if ( ! isset( $ud_data[ 'data' ][ $pa ] ) ) {
													...
													if ( ( int ) $pp === 1 ) {
														...
														$out[ 'out' ] = $this->parse_ud_data( $ud_data );
...
// 			echo '<pre>' . htmlspecialchars( print_r( $xml, TRUE ) ) . '</pre>'; exit;
	...
	public function parse_ud_data( & $ud_data = NULL, $ds_props_to_show = NULL ){
		...
		if ( is_array( $ud_data ) ) {
			...
			if ( $data_scheme ) {
				...
				$us_fields[ 'submit_datetime' ][ 'value' ] = $ud_data[ 'submit_datetime' ];
				...
				$us_fields[ 'mod_datetime' ][ 'value' ] = $ud_data[ 'mod_datetime' ];
					...
					if ( isset( $ds_props[ $key_2 ] ) ) {
						...
						if ( ! is_numeric( $key_2 ) ) {
							...
							if ( $ds_props[ $key_2 ][ 'field_type' ] == 'date' ){
								...
								$___date = explode( '-', $ds_prop_value );
								
								$format = '';
								
								$use_y = check_var( $ds_props[ $key_2 ][ 'sf_date_field_use_year' ] ) ? TRUE : FALSE;
								$use_m = check_var( $ds_props[ $key_2 ][ 'sf_date_field_use_month' ] ) ? TRUE : FALSE;
								$use_d = check_var( $ds_props[ $key_2 ][ 'sf_date_field_use_day' ] ) ? TRUE : FALSE;
								...
								$ds_prop_value = $___date[ 0 ] . '-' . $___date[ 1 ] . '-' . $___date[ 2 ];
								
								// -------------------------
								
								if ( ! empty( $format ) ) {
									
									$format = 'sf_us_dt_ft_pt_' . $format . '_' . $ds_props[ $key_2 ][ 'sf_date_field_presentation_format' ];
									
									$ds_prop_value =  strftime( lang( $format ), strtotime( $ds_prop_value ) );
	...
	public function parse_ds( & $data_scheme = NULL, $filter_params = FALSE ){
		...
		if ( is_array( $data_scheme ) AND key_exists( 'id', $data_scheme ) ){
			...
			// data submit
			$_ds_query_id = 'ud_ds_' . $data_scheme[ 'id' ] . '_data_submit_site';
			// data list
			$_dl_query_id = 'ud_ds_' . $data_scheme[ 'id' ] . '_data_list_site';
			...
			if ( $this->cache->cache( $_ds_query_id ) ) {
				...
				$this->db->like( 'params', '"submit_form_id":"' . $data_scheme[ 'id' ] . '"' );
				$this->db->limit( 1 );
			...
			if ( $_ds_menu_item ) {
				...
				$data_scheme[ 'site_link' ] = base_url() . $_ds_menu_item[ 'link' ];
			...
			if ( $_dl_menu_item ) {
				...
				$data_scheme[ 'data_list_site_link' ] = base_url() . $_dl_menu_item[ 'link' ];
				
			}
			else {
				
				$data_scheme[ 'data_list_site_link' ] = base_url() . 'submit_forms/index/miid/0/a/us/sfid/' . $data_scheme[ 'id' ]; 
			...
			if ( isset( $data_scheme[ 'params' ] ) ) {
				...
				if ( $filter_params ) {
					...
					if ( $_ds_menu_item ){
						...
						$data_scheme[ 'params' ] = filter_params( $data_scheme[ 'params' ], $_ds_menu_item[ 'params' ] );
					...
					if ( $_dl_menu_item ){
						...
						$data_scheme[ 'params' ] = filter_params( $data_scheme[ 'params' ], $_dl_menu_item[ 'params' ] );
