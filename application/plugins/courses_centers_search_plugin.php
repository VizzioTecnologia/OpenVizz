...
class Courses_centers_search_plugin extends Plugins_mdl{
	...
	public function run( &$data, $params = NULL ){
		...
		if ( is_object( $this->search ) ) {
			...
			log_message( 'debug', '[Plugins] Courses centers search plugin initialized' );
			...
			$plugin_params =									isset( $plugin_params[ 'courses_centers_search' ] ) ? $plugin_params[ 'courses_centers_search' ] : array();
			...
			$course_center_id =									( isset( $plugin_params[ 'course_center_id' ] ) AND is_numeric( $plugin_params[ 'course_center_id' ] ) ) ? $plugin_params[ 'course_center_id' ] : NULL;
