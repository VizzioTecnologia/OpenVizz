...
<section class="module-wrapper contact-module contact-module-wrapper <?= $module_data[ 'params' ][ 'module_class' ]; ?>">
	...
	?><div class="module-content contact-module-layout-<?= $module_data[ 'params' ][ 'contact_module_layout' ]; ?> contact-module-theme-<?= $module_data[ 'params' ][ 'contact_module_theme' ]; ?>" itemscope itemtype="http://schema.org/Organization">
		...
		<?php if ( isset( $module_data[ 'params' ][ 'contact_module_show_phones' ] ) AND $module_data[ 'params' ][ 'contact_module_show_phones' ] AND $contact[ 'phones' ] AND ! empty( $phones ) AND isset( $module_data[ 'params' ][ 'contact_module_phones_to_show' ] ) AND ! empty( $module_data[ 'params' ][ 'contact_module_phones_to_show' ] ) ) { ?>
		
		<p class="contact-phones contact-info-item">

			<?php if ( @$module_data[ 'params' ][ 'contact_module_show_phones_title' ] ) {?>
					...
					<span class="contact-module-phone">
						...
						<a href="tel://<?= str_replace( array( ' ', '-', '(', ')' ), '', $phone[ 'area_code' ] . $phone[ 'number' ] . $phone[ 'extension_number' ] ); ?>">
							...
							<span class="contact-module-phone-number" itemprop="telephone">
								...
								<?= ( check_var( $phone[ 'area_code' ] ) ? '<span class="area-code">(' . $phone[ 'area_code' ] . ')</span>' : '' ); ?> <span class="number"><?= $phone[ 'number' ]; ?></span> <span class="extension-number"><?= $phone[ 'extension_number' ]; ?></span>

							</span>

						</a>

						<?php if ( check_var( $module_data[ 'params' ][ 'contact_module_show_phones_titles' ] ) AND @$phone[ 'phone_title_publicly_visible' ] and @$phone[ 'title' ] ){ ?>
		...
		<?php if ( isset( $module_data[ 'params' ][ 'contact_module_show_emails' ] ) AND $module_data[ 'params' ][ 'contact_module_show_emails' ] AND $contact[ 'emails' ] AND ! empty( $emails ) AND isset( $module_data[ 'params' ][ 'contact_module_emails_to_show' ] ) AND ! empty( $module_data[ 'params' ][ 'contact_module_emails_to_show' ] ) ) { ?>

		<p class="contact-emails contact-info-item">

			<?php if ( @$module_data[ 'params' ][ 'contact_module_show_emails_title' ] ) {?>
			...
			<?php foreach ( $emails as $key => $email ) { ?>
				...
				<?php if ( in_array( $email[ 'key' ], $module_data[ 'params' ][ 'contact_module_emails_to_show' ] ) ) { ?>
					...
					<?php $email_title = ( ( @$module_data[ 'params' ][ 'contact_module_show_emails_titles' ] AND @$email[ 'email_title_publicly_visible' ] AND @$email[ 'title' ] ) ? TRUE : FALSE ); ?>
