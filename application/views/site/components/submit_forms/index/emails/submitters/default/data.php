...
<table width="80%" style="background: <?= $content_bg_color; ?>; color: <?= $content_fg_color; ?>; margin: 0 auto; text-align: left; border-collapse: collapse; border: none; width:80%" cellpadding="0" cellspacing="0">
	...
	foreach( $ud_data[ 'parsed_data' ][ 'full' ] as $alias => $pd ) {
		...
		if ( ! in_array( $alias, array( 'id', 'submit_datetime', 'mod_datetime' ) ) AND ( ( ( $pd[ 'value' ] == '' AND check_var( $params[ 'sfsmr_show_empty_fields' ] ) ) OR $pd[ 'value' ] != '' ) ) ) {
	...
	foreach( $ud_data[ 'parsed_data' ][ 'full' ] as $alias => $pd ) {
		...
		if ( ! in_array( $alias, array( 'id', 'submit_datetime', 'mod_datetime' ) ) AND ( ( ( $pd[ 'value' ] == '' AND check_var( $params[ 'sfsmr_show_empty_fields' ] ) ) OR $pd[ 'value' ] != '' ) ) ) {
			...
			if ( ! isset( $fields[ $alias ][ 'field_type' ] ) OR ! in_array( $fields[ $alias ][ 'field_type' ], array( 'button' ) ) ) {
				...
				echo '<td style="padding: ' . ( number_format( SUBMITTERS_DEFAULT_SPACING / 2, 2, '.', '' ) ) . 'em ' . ( number_format( SUBMITTERS_DEFAULT_SPACING, 2, '.', '' ) ) . 'em; width: 20%; text-align: right; ' . ( ( $current_row_index <= $total_rows ) ? 'border-bottom: thin solid ' . $content_border_color . '; ' : '' ) . 'border-right: thin solid ' . $content_border_color . '; border-left: ' . ( number_format( SUBMITTERS_DEFAULT_SPACING * 2, 2, '.', '' ) ) . 'em solid ' . $content_bg_color . ';">';
				...
				echo '<td style="padding: ' . ( number_format( SUBMITTERS_DEFAULT_SPACING / 2, 2, '.', '' ) ) . 'em ' . ( number_format( SUBMITTERS_DEFAULT_SPACING, 2, '.', '' ) ) . 'em; ' . ( ( $current_row_index <= $total_rows ) ? 'border-bottom: thin solid ' . $content_border_color . '; ' : '' ) . 'border-right: ' . ( number_format( SUBMITTERS_DEFAULT_SPACING * 2, 2, '.', '' ) ) . 'em solid ' . $content_bg_color . ';">';
