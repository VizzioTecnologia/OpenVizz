...
<span class="vui-interactive-el-wrapper vui-input-file-wrapper<?= $wrapper_class ? ' ' . $wrapper_class : ''; ?><?= $icon ? ' with-icon' : ''; ?>" <?= $_title ? $_title : ''; ?>><?php
	
	$input_params = array(
		
		'placeholder' => str_replace( array( '"', '\"' ), '\'', strip_tags( $text ) ),
		'name' => $name,
		'class' => 'el input-file input-box' . ( $class ? ' ' . $class : '' ),
		'id' => $id,
		'form' => $form,
		'autofocus' => $autofocus,
		'multiple' => $multiple,
	...
	echo form_upload( $input_params, $value );
