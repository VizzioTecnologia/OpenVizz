...
<span class="vui-interactive-el-wrapper vui-input-text-wrapper<?= $wrapper_class ? ' ' . $wrapper_class : ''; ?><?= $icon ? ' with-icon' : ''; ?>" <?= $_title ? $_title : ''; ?>><?php
	
	$input_params = array(
		
		'placeholder' => str_replace( array( '"', '\"' ), '\'', strip_tags( $text ) ),
		'name' => $name,
		'class' => 'el input-text input-box' . ( $class ? ' ' . $class : '' ),
		'id' => $id,
		'form' => $form,
		'autofocus' => $autofocus,
	...
	echo form_password( $input_params, $value );
