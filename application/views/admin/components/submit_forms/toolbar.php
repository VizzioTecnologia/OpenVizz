...
if ( isset( $submit_form[ 'id' ] ) ) {
	...
	$menu_items = array(
		...
		array(
			...
			'id' => 'parent',
			'title' => isset( $submit_form[ 'title' ] ) ? $submit_form[ 'title' ] : lang( 'submit_form' ),
			'link' => $submit_form[ 'edit_link' ],
			'icon' => 'submit-forms',
	...
	foreach( $submit_forms as $key => $_sf ) {
		
		if ( isset( $submit_form[ 'id' ] ) AND $submit_form[ 'id' ] != $_sf[ 'id' ] ) {
