		...
		<header>
			<h1><?php echo lang('edit_state'); ?> - <?php echo lang($country->title); ?></h1>
		</header>
		...
		<div>
			...
			<?php echo form_open(get_url('admin'.$this->uri->ruri_string())); ?>
				...
				<fieldset class="fl">
					...
					?>
					<?php echo form_dropdown('status', $options, set_value('status', $state->status),'id="status"'); ?>
					<?php $options = array(); ?>
					
					<?php echo form_error('title', '<div class="msg-inline-error">', '</div>'); ?>
					<?php echo form_label(lang('title')); ?>
					<?php echo form_input(array('id'=>'title','name'=>'title'),set_value('title', $state->title),'autofocus'); ?>
					
					<?php echo form_error('acronym', '<div class="msg-inline-error">', '</div>'); ?>
					<?php echo form_label(lang('acronym')); ?>
					<?php echo form_input(array('id'=>'acronym','name'=>'acronym'),set_value('acronym', $state->acronym)); ?>
