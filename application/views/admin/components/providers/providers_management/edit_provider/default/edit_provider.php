		...
		<header>
			<h1><?php echo lang('edit_provider'); ?></h1>
		</header>
			...
			<?= form_open( get_url( 'admin'.$this->uri->ruri_string() ), array( 'id' => 'provider-form', ) ); ?>
				...
				<fieldset class="fl">
					...
					<?php
						...
						endforeach;
					?>
					<?php echo form_dropdown('company_id', $options, set_value('company_id',$provider->company_id),'id="company-id"'); ?>
					<?php $options = array(); ?>
					
					<?php echo form_error('title', '<div class="msg-inline-error">', '</div>'); ?>
					<?php echo form_label(lang('title')); ?>
					<?php echo form_input(array('id'=>'title','name'=>'title','title'=>lang('tip_provider_title')),set_value('title',$provider->title),'autofocus'); ?>
					
					<?php echo form_error('default_provider_tax', '<div class="msg-inline-error">', '</div>'); ?>
					<?php echo form_label(lang('default_provider_tax')); ?>
					<?php echo form_input(array('id'=>'default-provider-tax','name'=>'default_provider_tax','title'=>lang('tip_default_provider_tax')),set_value('default_provider_tax',$provider->default_provider_tax)); ?>
