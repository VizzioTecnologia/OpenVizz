...
<div id="users-submits-search-results-wrapper" class="users-submits-wrapper results">
	...
	<?php if ( check_var( $users_submits ) ) { ?>
		...
		foreach ( $users_submits as $key => $user_submit ) {
			...
			?><div href="#<?= $us_hash; ?>" class="user-submit-wrapper modal <?= $_us_wrapper_class ?>" data-fancybox-title-id="title-<?= $us_hash ?>" data-autosize="1" data-fancybox-type="inline" data-fancybox-group="users-submits-<?= $unique_hash; ?>" >
				
				<div class="titles">
				<?php foreach ( $_titles_fields as $key_2 => $_title_field ) {
					
					if ( ! in_array( $key_2, array( 'capa', 'data-do-curso', ) ) ) {
				...
				<div id="<?= $us_hash; ?>" class="user-submit-wrapper <?= $_us_wrapper_class ?> user-submit-details-modal" style="display: none">
					...
					<span id="title-<?= $us_hash; ?>" style="display: none">
					<?php
						
						echo strip_tags( $modal_title, '<br><span>' );
